...
contract Staking {
    ...
    function stakeTokens( uint _amount) external {
        ...
        require(IERC20(kejiToken).balanceOf(msg.sender) >= _amount, "Insufficient Balance");
        require(IERC721(boredApe).balanceOf(msg.sender) > 0, "You must own a Bored Ape before you can stake");
    ...
    function stakingCommission() private view returns (uint256 commission) {
